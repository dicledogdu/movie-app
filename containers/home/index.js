...
function HomeContainer({ topRatedMovies=[], popularMovies=[], categories=[],selectedCategory}) {
    return ( <div>
        ...
        selectedCategory.movies.length > 0 && (
         <MovieSection title={categories.find((genre)=>`${genre.id}` === selectedCategory.id)?.name || "Bilinmiyor"
        } 
        movies={selectedCategory.movies}/>
        )}
        
        <MovieSection title='Popular Films' 
        movies={topRatedMovies.slice(1,7)}/>

        <MovieSection title='Your Favorites' 
